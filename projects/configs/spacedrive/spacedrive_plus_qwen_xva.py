...
vis_depth_tokens = False # pass depth maps through vision encoder and add to vision tokens (PE + DEPTH + VISION)
...
# ego status
ego_status = 'feature+PE' # 'feature', 'PE'. 'feature' means using a mlp to generate the ego status feature, 'PE' means using the past ego status as PE
