...
def main():
    ...
    if args.gpu_ids is not None:
        ...
        cfg.gpu_ids = [torch.device(f'cuda:{i}') for i in range(args.gpus)] if args.gpus is not None else [torch.device('cuda:0')]
    ...
    if args.launcher == 'none':
        ...
        cfg.gpu_ids = [torch.device(f'cuda:{i}') for i in range(world_size)]
