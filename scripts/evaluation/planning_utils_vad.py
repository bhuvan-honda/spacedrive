...
# ------------------------------------------------------------------------
# Modified from ST-P3 (https://github.com/OpenDriveLab/ST-P3)
# Copyright (c) OpenDriveLab. All rights reserved.
# ------------------------------------------------------------------------

"""
VAD metrics: Temavg / no yaw degree / resolution 200x200 / collision per frame
"""
import numpy as np
import torch
import cv2
...
class PlanningMetric:
    def __init__(self, base_path, step=6):
        ...
        self.nusc_maps = {
            "boston-seaport": NuScenesMap(
                dataroot=base_path, map_name="boston-seaport"
            ),
            "singapore-hollandvillage": NuScenesMap(
                dataroot=base_path, map_name="singapore-hollandvillage"
            ),
            "singapore-onenorth": NuScenesMap(
                dataroot=base_path, map_name="singapore-onenorth"
            ),
            "singapore-queenstown": NuScenesMap(
                dataroot=base_path, map_name="singapore-queenstown"
            ),
        }
    ...
    def get_birds_eye_view_label(
        ...
        if add_rec:
            gt_agent_fut_trajs = np.concatenate(
                [gt_agent_boxes[:, np.newaxis, 0:2], gt_agent_fut_trajs], 1
            )
            gt_agent_fut_yaw = np.concatenate(
                [gt_agent_boxes[:, np.newaxis, 6:7], gt_agent_fut_yaw], 1
            )
            gt_agent_fut_mask = np.concatenate(
                [np.ones_like(gt_agent_fut_mask[:, :1]), gt_agent_fut_mask], 1
            )
    ...
    def get_ego_seg(
        ...
        # move the ego car with gt_agent_boxes[:, np.newaxis, 0:2]  in the direction of yaw rotation
        gt_agent_fut_yaw = gt_agent_fut_yaw + gt_agent_boxes[:, np.newaxis, 6:7]

        ego_transition = gt_agent_boxes[0, np.newaxis, 0:2].repeat(T, axis=0)
        # convert ego_transition on to ego car coordinate to the global coordinate
        cos_yaw = np.cos(gt_agent_fut_yaw[0, :, 0])
        sin_yaw = np.sin(gt_agent_fut_yaw[0, :, 0])
        rot_mats = np.stack(
            [
        ...
        ego_transition = np.matmul(rot_mats, ego_transition[:, :, np.newaxis]).squeeze(
            -1
        )
        gt_agent_fut_trajs = ego_transition + gt_agent_fut_trajs
    ...
    def _get_poly_region_in_image(self, param):
        ...
        rot_mat_a = np.array(
            [[-np.sin(yaw_a), np.cos(yaw_a)], [np.cos(yaw_a), np.sin(yaw_a)]]
        )
    ...
    def traj_check(
        self, ego_seg, bev_seg, light_seg, drivable_seg, start_step=1, end_step=7
    ):
        ...
        out_of_drivable = (
            (np.expand_dims(drivable_seg, 0) == 0) & (ego_seg == 1)
        ).sum() > 0
    ...
    def red_light_area(self, curves):
        ...
        for curve in curves:
            left_lane, right_lane = expand_lane(
                curve[..., :2], width=2, res=res, dim=dim
            )
    ...
    def evaluate_single_coll(
        self, traj, segmentation, input_gt=None, gt_traj=None, index=None
    ):
        """
        traj: torch.Tensor (n_future, 2)
            the trajectory is in the self car IMU coordinate system
        ...
        for i in range(traj.size(0)):
            ...
            tmp = rc_ori @ rot_mat.cpu().numpy() - (
                self.bx.cpu().numpy() / self.dx.cpu().numpy()
            )
