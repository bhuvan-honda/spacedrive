...
class PlanningMetric():
    ...
    def get_ego_seg(
        ...
        rot_mats = np.stack([np.stack([cos_yaw, -sin_yaw], axis=-1),
                            np.stack([sin_yaw, cos_yaw], axis=-1)], axis=-2) #(T,2,2)
        ego_transition = np.matmul(rot_mats, ego_transition[:,:,np.newaxis]).squeeze(-1)
