...
from planning_utils_vad import PlanningMetric
import torch
...
def process_data(preds, start, end, key_infos, metric_dict, lock, pbar, planning_metric, visualize=False, vis_path='results_planning_only/'):
    ...
    for i in range(start, end):
        try:
            ...
            # set yaw to zero
            pred_traj_yaw = np.concatenate([pred_traj, np.zeros_like(pred_traj[..., :1])], axis=-1)
            ...
            if fut_valid_flag:
                ...
                for i in range(future_second):
                    ...
                    if out_of_drivable and not rec_out:
                        # calculate how many frames from the ego_seg drive out of road
                        out_of_drivable_seg =  ((np.expand_dims(drivable_seg, 0) == 0) & (ego_seg[1:cur_time+1] == 1))
                        out_of_frame = out_of_drivable_seg.any(axis=(1, 2))
                        metric_dict['plan_boundary_{}s'.format(i+1)]  = out_of_frame.mean().item()
                    ...
                    if out_of_drivable or obj_box_coll.max().item() > 0:
                        print(f"Predicted Trajectory: {pred_traj.numpy()}"
                               ...
                               + f"Out of Drivable Area for {cur_time}s: {out_of_frame}, Recovery: {rec_out} "                            )
                    
                if visualize:
                    visualize_bev(bev_seg, drivable_seg, ego_seg, gt_traj.numpy(), pred_traj.numpy(), obj_box_coll, out_of_drivable, data, vis_path)
...
def main(args):
        ...
        if os.path.exists(pred_path+data['token']):
            with open(pred_path+data['token'],'r',encoding='utf8')as f:
                ...
                if full_match:
                    ...
                    # Append the trajectory to all_trajectories
    ...
    vis_path = '/data/sandbox/workspace/user/zhenghaoz/peadvlm/test/noPEinput/vis/' + pred_path.split('/')[-5] + '/' + pred_path.split('/')[-3] + '/' + pred_path.split('/')[-2] + '/'
